...
This table contains the metadata describing the source. Statistics is either derived from entities, in which cas `statistic_id` is equivalent to the `entity_id`, or imported from an external source. Statistics imported from an external source is similar to `entity_id`, but uses a `:` instead of a `.` as a delimiter between the domain and object.
